...
function IconBook({ active }) {
  return (
    <svg
      className={`w-6 h-6 transition-colors ${active ? 'text-sky-500' : 'text-slate-400 dark:text-slate-500'}`}
...
function SyncIndicator({ status, connected }) {
  if (status === 'syncing') {
    return (
  ...
  if (status === 'error') {
    return <span className="text-red-400 text-xs font-medium">Sync error</span>
  }
  if (!connected) {
    return <span className="text-slate-500 text-xs font-medium">Offline</span>
  }
  return null
}
...
export default function App() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full bg-slate-50 dark:bg-gray-950">
        <div className="text-center">
          <div className="w-10 h-10 border-2 border-sky-500 border-t-transparent rounded-full animate-spin mx-auto" />
          <p className="mt-4 text-slate-400 dark:text-slate-500 text-sm tracking-wide">Loading logbook</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-gray-950">
      ...
      {/* Header */}
      <header className="bg-white dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800 px-5 pt-safe flex-shrink-0">
        <div className="flex items-center justify-between py-3">
          <div className="flex items-center gap-2.5">
            {/* Aviation-themed wordmark */}
          ...
          <SyncIndicator status={syncStatus} connected={connected} />
        </div>
        {/* Accent line */}
        <div className="h-px bg-gradient-to-r from-sky-500/60 via-sky-400/30 to-transparent -mx-5" />
      </header>
      ...
      {/* Bottom navigation */}
      {page !== 'form' && connected && (
        <nav className="bg-white dark:bg-gray-900 border-t border-slate-200 dark:border-gray-800 flex flex-shrink-0 pb-safe">
